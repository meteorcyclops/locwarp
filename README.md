...
本專案於 WiFi Tunnel 模式下需以**系統管理員權限**執行,以建立 TUN 虛擬網路介面並與 iOS 裝置協商 RSD(Remote Service Discovery)通道。雖然程式碼已經內部測試,但開發者不保證於所有 Windows 版本、硬體組合、網路環境下皆能穩定運行。使用者應自行承擔因操作不當、軟體衝突、驅動程式異常所導致之裝置連線中斷、系統不穩、資料遺失或裝置配對記錄損毀等風險。
